...
def raise_complaint(request):
    if request.method == "POST":
        ...
        image = request.FILES.get('image')  # Handle uploaded image if request.method == "POST":
        ...
        if image_data:
            ...
            ext = format.split('/')[-1] 
            image = ContentFile(base64.b64decode(imgstr), name=f"complaint_image.{ext}")

            # Save the complaint with the image (assuming a model Complaint)
...
def upload_image(request):
    if request.method == "POST" and request.FILES.get("image"):
        ...
        unique_name = f"{uuid.uuid4().hex}_{image.name}"
        save_path = os.path.join(settings.MEDIA_ROOT, "complaints/images/", unique_name)
        ...
        # Optionally, save to the database if required
        # For now, return the image name/path
        return JsonResponse({"image_name": unique_name})
...
def notify_user(request):
    if request.method == "POST":
        ...
        if 'captured_image' in request.FILES:
            image_file = request.FILES['captured_image']
            # Generate a unique file name
            unique_filename = f"{uuid.uuid4()}.png"
            # Define the folder to store images
            folder_path = os.path.join(settings.MEDIA_ROOT, 'complaints/images')
            ...
            with open(image_path, 'wb') as f:
                for chunk in image_file.chunks():
                    f.write(chunk)
            # Save the image path to the complaint (assuming a field exists)
            complaint.image_path = f"complaints/images/{unique_filename}"
            complaint.save()
